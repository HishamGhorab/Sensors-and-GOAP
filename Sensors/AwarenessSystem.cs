...
public class TrackedTarget
{
    ...
    {
        //stores the old awareness value
        var oldAwareness = Awareness;

        //set the detectable target if the target hasnt been set before
        if (target != null) 
            Detectable = target;
        ...
        //if old awareness is less than 2, but the new awareness is more than 2. What does this mean?
        //it means if the value is getting updated? right? its going from 1 - 2 forexample
...
public class AwarenessSystem : MonoBehaviour
{
    ...
    void Update()
    {
        //cleans up targets that have 0 awareness value
